...
class PagesApi {
    ...
    async uploadImage(id, body) {
        return await request(`${config.serverAPIurl}/${this.pathName}/${id}/image`, `POST`, body, {}, 'multipart/form-data');
    }
    
    async updateImage(id, alt) {
        return await request(`${config.serverAPIurl}/${this.pathName}/${id}/image`, `PUT`, {alt: alt});
    }
