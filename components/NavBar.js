    ...
    <div className={styles.wrapper}>
      <div className={styles.container}>
        ...
        <form onSubmit={handleSubmit} style={{ position: 'relative', display: 'flex' }}>
          <input
            ...
            type="search"
            placeholder="Например: Салют"></input>
          <button
      ...
      <div className={styles.container}>
        <div className={styles.nav}>
          <Link href={user.token ? '/login/accaunt' : '/login'}>
        ...
        <Link href="/">
          <a>
            <p className={styles.userAgreement}>
              Политика конфеденциальности и пользовательского соглашения
            </p>
          </a>
        </Link>

        <p className={styles.userAgreement}>© ООО “Пиро-Дон” 2021 г.</p>
      </div>
    </div>
  );
}
