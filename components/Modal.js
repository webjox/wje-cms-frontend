...
export default function Modal({showModal, rowTwo,showModalHandler,title, children, option}) {
    if(option){
        return (
            <div className={styles.shopsModal} style={showModal ? {transform: 'scale(1) translate(-50%, -50%)'} : {transform: 'scale(0) translate(-50%, -50%)'}}>
                <div className={styles.ModalTitle}>
                    <p>{title}</p>
                    <button onClick={showModalHandler}><img src='/close.svg'/></button>
                </div>
