...
export default function ControllerMenu({ filters, handleFilters }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', color: '#1d1d1d' }}>
      ...
      <div style={{ height: '50px', padding: '20px', display: 'flex', alignItems: 'center' }}>
        <SettingsIcon />
        <Link href="/admin/orders/statuses">Менеджер статусов</Link>
      </div>
      <div style={{ height: '50px', padding: '20px', display: 'flex', alignItems: 'center' }}>
        <PaymentIcon />
        <Link href="/admin/orders/paymentMethods">Способы оплаты</Link>
      </div>
      <div style={{ height: '50px', padding: '20px', display: 'flex', alignItems: 'center' }}>
        <LocalShippingIcon />
        <Link href="/admin/orders/shippingMethods">Способы доставки</Link>
      </div>
      <div style={{ padding: '20px' }}>
        <FormDialog title={'Создать заказ'} action={createOrder} />
      </div>
