...
export default function ProductCard({product, index, changeQuantity, updateQuantity, deleteItem}) {
                ...
                <FormDialog 
                    ...
                    fields={
                        <div style={{display: 'flex', flexDirection:'column', width: '500px'}}>
                            <TextField type='number' id="quantity" label="quantity" defaultValue={product.quantity} onChange={e => changeQuantity(e, index)} />
                        </div>
                    }
                    action={e => updateQuantity(index)}
                />
                <Button onClick={e => deleteItem(product._id, index)} color="secondary">Удалить товар</Button>
                </div>
                </Popover>
            </div>
        </div>
