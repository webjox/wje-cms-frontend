...
export default function ControllerMenu({ manufacturers }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', color: '#1d1d1d' }}>
      ...
      <div style={{ height: '50px', padding: '20px', display: 'flex', alignItems: 'center' }}>
        <SettingsIcon />
        <Link href="/admin/products/categories">Редактировать группы</Link>
      </div>
      <div style={{ height: '50px', padding: '20px', display: 'flex', alignItems: 'center' }}>
        <SettingsIcon />
        <Link href="/admin/products/tags">Редактировать тэги</Link>
      </div>
      <div style={{ height: '50px', padding: '20px', display: 'flex', alignItems: 'center' }}>
        <ImportExportIcon />
        <Link href="/admin/products/import">Импорт товаров</Link>
      </div>
