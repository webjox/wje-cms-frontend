  ...
  return (
    <div className={styles.sliderWrapper}>
      <div className={styles.container} style={{ transform: `translateX(${shift}px)` }}>
        {video ? (
          <div className={styles.contantWrapper}>
            <YouTube
              videoId={video.replace('https://youtu.be/', '')}
              opts={opts}
              onReady={_onReady}
            />
          </div>
      ...
      <div className={styles.sliderControler}>
        <div onClick={handleLeftClick} className={styles.slideControl}>
          {' '}
          <img style={{ transform: 'rotate(180deg)' }} src="/toView.svg" />{' '}
        </div>
        <div onClick={handleRightClick} className={styles.slideControl}>
          {' '}
          <img src="/toView.svg" />{' '}
        </div>
      </div>
