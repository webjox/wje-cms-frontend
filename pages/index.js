                    ...
                    <div className={styles.category} key={index}>
                        ...
                        {state.categories.map((subCategory,index) => {
                            if(subCategory.parent_id === category._id){
                                return <CategoryCard key={index} data={subCategory} link={subCategory._id} title={subCategory.name} />    
                            }
                        })}
                    </div>
                )
    ...
    const serviceCardRender = () => {
        return pages.map((item, index) => {
            if(!item.is_system && item.enabled && !item.in_the_sidebar){  
                return(
                    <div key={index}>
                        <ServiceCard title={item.card_name} background={item.card_image.url} link={item._id}/> {/* Добавь background */}
                    </div>
                )
            }
        })
