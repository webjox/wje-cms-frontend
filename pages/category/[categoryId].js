...
const Post = () => {
  ...
  useEffect(async () => {
    if (categoryId) {
      ...
      const id = newFilter.findIndex(item => item[0] === 'categoryId');
      if (id != -1) {
        newFilter.splice(id, 1);
      }
      newFilter.push(['categoryId', categoryId]);
      state.setFilterData(newFilter);
      setCategory(await state.getCategoryId(categoryId));
    }
  }, [categoryId]);

  useEffect(async () => {
    if (category.parent_id) {
      setParentCategory(await state.getCategoryId(category.parent_id));
    }
  }, [category]);
  ...
  const tagReset = () => {
    const newFilter = [...state.filter];
    const id = newFilter.findIndex(item => {
      return item[0] === 'tags';
    });
    if (id != -1) {
      newFilter.splice(id, 1);
      ...
      <div className={styles.main}>
        <div className={styles.tags}>
          ...
          <div className={styles.tagsWrapper}>
            <Carousel id="asfaca">{tagsRender()}</Carousel>
          </div>
        </div>
        <div className={styles.breadcrumbs}>
          ...
          <a onClick={tagReset}>{category.name}</a>
        </div>
        ...
        {products.length != 0 ? (
          ...
          'К сожалению данного сочетания товара нет'
        )}
      </div>
