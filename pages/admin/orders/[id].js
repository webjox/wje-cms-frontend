...
import { Button, Chip, FormControlLabel, Input, InputLabel, MenuItem, Select, Switch, TextField } from "@material-ui/core";
...
const leftSideRender = (id, order, setOrder, handleChange, handleFlagChange, statuses, paymentMethods, shippingMethods) => {
    ...
    <div className={styles.infoContainer}>
        <div className={styles.orderInfo}>
            <span className={styles.field} >Дата оформления заказа: <span>{formateDate(order.date_placed)}</span> </span>
            <span className={styles.field} >Статус: {status ? <Chip label={status.name} style={{width: 'max-content', color: `${status.color}`, background: `${status.bgcolor}`, marginLeft: 10}} /> : null}</span>
            <span className={styles.field} >Дата оплаты заказа: <span>{formateDate(order.date_paid)}</span></span>
            <span className={styles.field} >Метод доставки: {getMethodName(shippingMethods, order.shipping_method_id)} </span>
            <span className={styles.field} >Метод оплаты: {getMethodName(paymentMethods, order.payment_method_id)} </span>
            <span className={styles.field} >Заметка: <span>{order.note}</span></span>
            <FormDialog
                title={'Редактировать заказ'}
                fields={
                    <div style={{display: 'flex', flexDirection:'column', width: '500px'}}>
                    <TextField className={formStyles.formInput} type='text' id="note" label="Заметка" defaultValue={order.note} onChange={e => handleChange(e, 'note')} />
                    <InputLabel className={formStyles.formInput} id="demo-mutiple-chip-label">Статус</InputLabel>
                    <Select
                    ...
                    <InputLabel className={formStyles.formInput} id="payment-method-controller">Способ оплаты</InputLabel>
                    <Select
                    ...
                    <InputLabel className={formStyles.formInput} id="shipping-method-controller">Способ доставки</InputLabel>
                    <Select
                ...
                action={e => {updateOrder(id, order, setOrder)}}
            />
        </div>

        <span>Заказчик</span>
        <div className={styles.customerInfo}>
            {order.customer_id ? <a style={{color: '#3f51b5'}} href={`/admin/customers/${order.customer_id}`}>{order.email}</a> : <span>{order.email}</span>}
            <span className={styles.field} >{order.shipping_address.full_name}</span>
            <span className={styles.field} >{order.mobile}</span>
            <span className={styles.field} >Адрес доставки: <span>{`${order.shipping_address.state}, ${order.shipping_address.city}, ${order.shipping_address.address1}, ${order.shipping_address.postal_code}`}</span></span>
            <FormDialog
                title={'Редактировать пользователя'}
                fields={
                    <div style={{display: 'flex', flexDirection:'column', width: '500px'}}>
                    <TextField type='email' id="email" label="Email-пользователя" defaultValue={order.email} onChange={e => {handleChange(e, 'email')}} />
                    <br />
                    <TextField type='text' id="full_name" label="Имя пользователя" defaultValue={order.shipping_address.full_name} onChange={e => {handleChange(e, 'shipping_address', 'full_name')}} />
                    <br />
                    <TextField type='mobile' id="mobile" label="Номер телефона" defaultValue={order.mobile} onChange={e => {handleChange(e, 'mobile')}} />
                    <br />
                    <span>Адрес доставки:</span>
                    <TextField type='text' id="state" label="Регион" defaultValue={order.shipping_address.state} onChange={e => {handleChange(e, 'shipping_address', 'state')}} />
                    <TextField type='text' id="city" label="Город" defaultValue={order.shipping_address.city} onChange={e => {handleChange(e, 'shipping_address' ,'city')}} />
                    <TextField type='text' id="address1" label="Адрес" defaultValue={order.shipping_address.address1} onChange={e => {handleChange(e, 'shipping_address', 'address1')}} />
                    <TextField type='text' id="postal_code" label="Почтовый индекс" defaultValue={order.shipping_address.postal_code} onChange={e => {handleChange(e, 'shipping_address', 'postal_code')}} />
                    </div>
                }
    ...
    return (
        <div className={styles.infoContainer}>
            ...
            {order.items.map((item, index) => {
                return <ProductCard key={index} index={index} product={item} changeQuantity={changeQuantity} updateQuantity={updateQuantity} deleteItem={deleteItem} />
            })}
            </div>

            <div style={{display: 'flex', flexDirection: 'column', alignItems: 'flex-end', padding: 20}}>
...
export default function Order () {
    ...
    return (<div>
        <Header 
            pageName={`Редактирование заказа ${order ? order.number : ''}`} 
            backButton={true}
        />
        {order ? 
        <div>
            <ContentContainer
            left={leftSideRender(id, order, setOrder, handleChange, handleFlagChange, statuses, paymentMethods, shippingMethods)}
