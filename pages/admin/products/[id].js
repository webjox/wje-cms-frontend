...
import { Button, Chip, Divider, Input, InputLabel, MenuItem, NativeSelect, Select, TextField, Menu, FormControlLabel, Switch } from "@material-ui/core";
...
export default function EditProduct() {
    ...
    return (
        <div>
        <Header 
            pageName={`Редактирование товара ${product ? product.name : ''}`} 
            backButton={true}
        />
        {product ?
        <div>
            <form className={`container ${formStyles.formContainer}`}>
                ...
                <TextField className={formStyles.formInput} id="name" label="Наименование товара" defaultValue={product.name} onChange={event => {handleChange(event, 'name')}} />
                <TextField className={formStyles.formInput} id="sku" label="Артикул" defaultValue={product.sku} onChange={event => {handleChange(event, 'sku')}} />
                ...
                <InputLabel className={formStyles.formInput} htmlFor="category">Родительская категория</InputLabel>
                <NativeSelect
                ...
                {categoriesData.map((item, index) => {
                    return <option key={index} aria-label={item.name} value={item._id}>{item.name}</option>
                })}
                </NativeSelect>
                <FormControlLabel
                ...
                label="Enabled"
                />
                <InputLabel className={formStyles.formInput} htmlFor="tags-controller">Эффекты</InputLabel>
                <Select
                ...
                <TextField className={formStyles.formInput} id="volleys" label="Количество залпов (опционально)" defaultValue={product.volleys} onChange={event => {handleChange(event, 'volleys')}} />
                <div className={formStyles.formButtonContainer}>
                    <Button onClick={e => {updateMainInfo(id, product)}} variant="contained" color="primary">Сохранить</Button>
                </div>
            </form>
            ...
            />
            <div className={formStyles.formButtonContainer}>
                    <Button onClick={e => {updateDescription(id, product)}} variant="contained" color="primary">Сохранить</Button>
            </div>        
            </form>
            ...
            <form className={`container ${formStyles.formContainer}`}>
                ...
                <div className={formStyles.formButtonContainer}>
                    <Button onClick={e => {updateTags(id, product.tags)}} variant="contained" color="primary">Сохранить</Button>
                </div>
            </form>
            ...
            <form className={`container ${formStyles.formContainer}`}>
                ...
                <span style={{marginTop: 20}}>Общее количество товара: {getProductQuantity(product)}</span>
                ...
                >
                    {shopsData.map(item => {
                        ...
                        if(existShop.length === 0) return <MenuItem onClick={e => {addShopHandler(item); handleClose()}}>{item.name}</MenuItem>
                    })}
                </Menu>
                ...
                <TextField className={formStyles.formInput} id="package_quantity" label="Количество товара в упаковке" defaultValue={product.package_quantity} onChange={event => {handleChange(event, 'package_quantity')}} />
                <div className={formStyles.formButtonContainer}>
                    <Button onClick={e => {updateQuantity(id, product)}} variant="contained" color="primary">Сохранить</Button>
                </div>
            </form>
            ...
            <form className={`container ${formStyles.formContainer}`}>
                ...
                <TextField className={formStyles.formInput} id="stock_price" type='number' label="Стоимость товара со склада" defaultValue={product.stock_price} onChange={event => {handleChange(event, 'stock_price')}} />
                <TextField className={formStyles.formInput} id="price" type='number' label="Стоимость товара в магазинах" defaultValue={product.price} onChange={event => {handleChange(event, 'price')}} />
                <div className={formStyles.formButtonContainer}>
                    <Button onClick={e => {updateCost(id, product)}} variant="contained" color="primary">Сохранить</Button>
                </div>
            </form>
            ...
            <form className={`container ${formStyles.formContainer}`}>
                ...
                />
                <div onClick={e => {addOptionHandler()}} style={{alignSelf: 'center', cursor: 'pointer'}}><AddCircleIcon color='primary' style={{fontSize: 48}} /></div>
            </form>
            ...
            <form className={`container ${formStyles.formContainer}`}>
                ...
                <TextField className={formStyles.formInput} id="meta_title" label="Meta title" defaultValue={product.meta_title} onChange={event => {handleChange(event, 'meta_title')}} />
                <TextField className={formStyles.formInput} id="meta_description" label="Meta description" defaultValue={product.meta_description} onChange={event => {handleChange(event, 'meta_description')}} />
                <TextField className={formStyles.formInput} id="slug" label="URL" defaultValue={product.slug} onChange={event => {handleChange(event, 'slug')}} />
                <div className={formStyles.formButtonContainer}>
                    <Button onClick={e => {updateMeta(id, product)}} variant="contained" color="primary">Сохранить</Button>
                </div>
            </form>
            ...
            <form className={`container ${formStyles.formContainer}`}>
                ...
                <div style={{display: 'flex', flexWrap: 'wrap'}}>{productImages.map((item, index) => {
                   return <ImageCard key={index} image={item} removeAction={removeImageHandler} updateAlt={updateImageHandler} />
                })}</div>
                ...
                <DropzoneArea
                    ...
                    filesLimit={1}
                />
                <Button onClick={e => { addImage(id, imageData, setProductImages)}} variant="contained" color="primary">Сохранить</Button>
                
                <span style={{marginTop: 50}}>Загруженные файлы:</span>
                <div style={{display: 'flex', flexWrap: 'wrap'}}>{productFiles.map((item, index) => {
                ...
                />
                <Button onClick={e => { addFile(id, fileData, setProductFiles) }} variant="contained" color="primary">Сохранить</Button>
                <span style={{marginTop: 30}}>Видео</span>
                <TextField className={formStyles.formInput} id="video" label="youtube video" defaultValue={product.video} onChange={event => {handleChange(event, 'video')}} />
                <Button onClick={e => {updateVideo(id, product)}}  variant="contained" color="primary">Сохранить</Button>
            </form>
            ...
            <Button onClick={e => {
                ...
                }} className={formStyles.DeleteButton} variant="contained" color="secondary">Удалить товар</Button>
            </div>
        </div>
